...
from the course API, carries your harness forward from last week, and helps
you turn the spec's checkable lines into tests of your own. Read the spec
before you build, and see `spec/README.md` for how the checks in this repo
relate to it.
...
and every page that uses `Layout` picks up the same head block automatically.
...
This repo has already made that swap: `astro.config.mjs` sets
`base: "/comp4020-crit4-jaz0502/"` to match the Pages URL, pages live in
`src/pages/`, and `typecheck` runs `astro check` instead of a bare `tsc`.
Astro only prefixes `base` onto what it generates itself (styles, scripts,
`astro:assets`) --- a hand-written `<a href="/foo">` in markup won't get it,
so internal links stay relative (`href="./"`) rather than root-absolute. If a
page nests under a subpath (like `court-bookings/` did last time), the
relative link back to `./` or `../` has to change per page --- see how
`Layout.astro` did it last week if you need that again.
